...
var PRODUCTS = [
    ...
    // Add new products here — use Cloudinary URLs only
];
...
function initPage() {
    updateCartCount();
    renderFeaturedProductsWithSkeleton();  // Feature 4
    initModals();
    initHamburger();
    initSmoothScroll();
    initNewsletter();
    initReviews();
    initQuickViewModal();                  // Feature 2
    // Sticky cart bar is products-page only — not needed on homepage
}
