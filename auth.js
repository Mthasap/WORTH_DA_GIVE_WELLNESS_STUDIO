...
/* -- Inject account dropdown + auth modal styles (self-contained) -- */
(function injectAuthStyles() {
    if (document.getElementById('wdg-auth-styles')) return;
    var s = document.createElement('style');
    s.id = 'wdg-auth-styles';
    s.textContent = [
      /* Hidden utility - applies to auth elements only to avoid conflicts */
      ".auth-modal-overlay.hidden, #authModal.hidden { display:none !important; }",
      ".auth-panel.hidden { display:none !important; }",
      ".auth-msg.hidden { display:none !important; }",
      ".auth-btn-spinner.hidden { display:none !important; }",
      ".account-dropdown.hidden { display:none !important; }",

      /* Account dropdown */
      ".account-dropdown { position:absolute; top:calc(100% + 10px); right:0; min-width:230px; background:#fff; border-radius:12px; box-shadow:0 8px 32px rgba(0,0,0,0.18); border:1px solid #e8e8e8; z-index:9999; overflow:hidden; animation:wdgDropIn 0.18s ease; }",
      "@keyframes wdgDropIn { from{opacity:0;transform:translateY(-8px)} to{opacity:1;transform:translateY(0)} }",
      ".account-dropdown-header { padding:14px 16px 12px; background:linear-gradient(135deg,#2c5530,#4CAF50); color:#fff; }",
      ...
      ".account-dropdown-item { display:flex; align-items:center; gap:10px; padding:11px 16px; color:#333; text-decoration:none; font-size:0.88rem; font-weight:500; transition:background 0.15s; cursor:pointer; }",
      ...
      "#loginBtn { display:flex !important; align-items:center !important; gap:5px !important; white-space:nowrap !important; }",

      /* Auth modal */
      "#authModal { position:fixed; inset:0; background:rgba(0,0,0,0.55); z-index:10000; display:flex; align-items:center; justify-content:center; padding:16px; }",
      ".auth-modal-content { background:#fff; border-radius:16px; padding:2rem; width:100%; max-width:440px; position:relative; box-shadow:0 12px 48px rgba(0,0,0,0.22); animation:wdgModalIn 0.2s ease; max-height:92vh; overflow-y:auto; }",
      "@keyframes wdgModalIn { from{opacity:0;transform:scale(0.95)} to{opacity:1;transform:scale(1)} }",
      ".auth-modal-close { position:absolute; top:12px; right:14px; background:none; border:none; font-size:1.6rem; cursor:pointer; color:#aaa; line-height:1; padding:4px 8px; z-index:1; }",
      ".auth-modal-close:hover { color:#333; }",

      /* Auth tab bar */
      ".auth-tab-bar { display:flex; border-bottom:2px solid #eee; margin-bottom:1.4rem; }",
      ".auth-tab { flex:1; padding:10px; text-align:center; font-weight:700; font-size:0.95rem; color:#999; cursor:pointer; border:none; background:none; border-bottom:2px solid transparent; margin-bottom:-2px; transition:color 0.2s,border-color 0.2s; }",
      ...
      ".auth-switch-link { text-align:center; margin-top:0.8rem; font-size:0.85rem; color:#888; }",
      ".auth-switch-link a { color:#4CAF50; font-weight:600; }",

      /* Auth form fields */
      ".auth-field { margin-bottom:1rem; }",
      ".auth-field label { display:flex; align-items:center; gap:6px; font-size:0.82rem; font-weight:700; color:#555; margin-bottom:6px; }",
      ".auth-field input { width:100%; padding:11px 14px; border:2px solid #ddd; border-radius:8px; font-size:0.95rem; outline:none; transition:border-color 0.2s; box-sizing:border-box; }",
      ".auth-field input:focus { border-color:#4CAF50; }",
      ".auth-pw-wrap { position:relative; }",
      ".auth-pw-wrap input { padding-right:42px; }",
      ".auth-pw-toggle { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; color:#aaa; padding:0; display:flex; }",
      ".auth-pw-toggle:hover { color:#555; }",

      /* Submit button */
      ".auth-submit { width:100%; padding:13px; background:#4CAF50; color:#fff; border:none; border-radius:8px; font-size:1rem; font-weight:700; cursor:pointer; transition:background 0.2s; display:flex; align-items:center; justify-content:center; gap:8px; margin-top:0.5rem; }",
      ".auth-submit:hover:not(:disabled) { background:#388e3c; }",
      ".auth-submit:disabled { opacity:0.65; cursor:not-allowed; }",

      /* Spinner */
      ".auth-btn-spinner { width:18px; height:18px; border:2px solid rgba(255,255,255,0.4); border-top-color:#fff; border-radius:50%; animation:wdgSpin 0.8s linear infinite; }",
      ...
      ".auth-msg.ok, .auth-msg.success { background:#d4edda; color:#155724; }",
      ".auth-msg.err, .auth-msg.error { background:#f8d7da; color:#721c24; }",

      /* Register extras */
      ".reg-checkbox-group { display:flex; align-items:flex-start; gap:10px; margin-bottom:0.8rem; font-size:0.85rem; color:#555; }",
      ".reg-checkbox-group input[type=checkbox] { margin-top:3px; flex-shrink:0; width:16px; height:16px; }",
      ".reg-age-hint { font-size:0.78rem; color:#888; margin-top:4px; }",
      ".reg-consequences-box { background:#fff8e1; border:1px solid #ffe082; border-radius:8px; padding:12px 14px; font-size:0.82rem; color:#555; margin-bottom:1rem; line-height:1.5; }",
      ".reg-warn-head { font-weight:700; color:#e65100; margin-bottom:6px; display:flex; align-items:center; gap:6px; }",

      /* Toast - matches existing JS class names */
      ".toast { position:fixed; bottom:24px; left:50%; transform:translateX(-50%) translateY(20px); background:#333; color:#fff; padding:12px 24px; border-radius:30px; font-size:0.9rem; font-weight:600; z-index:99999; opacity:0; transition:opacity 0.3s,transform 0.3s; pointer-events:none; max-width:90vw; text-align:center; }",
      ".toast.toast-success { background:#2c5530; }",
      ".toast.toast-show { opacity:1; transform:translateX(-50%) translateY(0); }",

      /* Verify panel */
      ".auth-verify-icon { width:56px; height:56px; background:#e8f5e9; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 1rem; color:#4CAF50; }",

      /* Mobile */
      "@media(max-width:480px) { .auth-modal-content { padding:1.5rem 1rem; } }"
    ].join("\n");
...
function showAuthMsg(elId, text, type) {
    ...
    if (!el) return;
    el.textContent = text;
    // normalise class names: accept 'error'/'success' or 'err'/'ok'
    var cls = type === 'error' ? 'err' : type === 'success' ? 'ok' : type;
